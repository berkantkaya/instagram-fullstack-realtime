...
export const updateProfileUser=({userData,avatar,auth})=>async(dispatch)=>{
    ...
    return dispatch({
        type:GLOBALTYPES.ALERT,payload:{error:"Please add your fullname"}
    })

    if(userData.fullname.length>25)
    return dispatch({
        type:GLOBALTYPES.ALERT,payload:{error:"Your fullname too long."}
    })

    if(!userData.story.length>200)
    return dispatch({
        type:GLOBALTYPES.ALERT,payload:{error:"Your story too long."}
    })
