...
const postCtrl={
    ...
    likePost:async(req,res)=>{
        try {
          ...
          if(!like) return res.status(400).json({msg:"This post not exist."})
    ...
    getPost:async(req,res)=>{
        try {
         const post=await Posts.findById(req.params.id).populate("user likes","avatar username fullname followers").populate({
            path:"comments",
            populate:{
               path:"user likes",
               select:"-password"  
        ...
        if(!post) return res.status(400).json({msg:"This post not exist."})
    ...
    savePost:async(req,res)=>{
        try {
          ...
          if(!save) return res.status(400).json({msg:"This user not exist."})
